...
/**** Link Cutting or Freezing Process. *******/	
	...
	for(int i=0;i<L;i++){
		for(int j=0;j<L;j++){
			...
			level[i*L+j]= 0;		// initial stete: all nodes are considerd to be root.
...
int main(){
	...
	double e_step, e_av, e_sqd_av;      // average E per spin, squared average
	double m_step, m_av, m_sqd_av;   
	double c; double chi;
	...
	cout << "T\t"<< "<e>\t"<< "<e^2>\t"<< "<m>\t"<< "<m^2>"<< "c\t"<< "chi"  << endl;
	 for(int k = 0; k<n_T;k++){
		...
		init_S();	// set spins, which are all up.
